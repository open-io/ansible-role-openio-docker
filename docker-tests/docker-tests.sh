...
configure_environment() {
  ...
  case "${DISTRIBUTION}_${VERSION}" in
    ...
    'centos_8'|'centos_7'|'fedora_25')
      ...
      run_opts+=('--privileged=true' '--volume=/sys/fs/cgroup:/sys/fs/cgroup:ro')
      ;;
    'ubuntu_14.04')
      #run_opts+=('--volume=/sys/fs/cgroup:/sys/fs/cgroup:ro')
    ...
    'ubuntu_18.04'|'ubuntu_16.04'|'debian_8')
      run_opts=('--privileged=true' '--volume=/run' '--volume=/run/lock' '--volume=/tmp' '--volume=/sys/fs/cgroup:/sys/fs/cgroup:ro' '--cap-add=NET_ADMIN' '--cap-add=SYS_ADMIN' '--cap-add=SYS_RESOURCE')
